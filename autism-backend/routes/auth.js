...
router.post('/signup', async (req, res) => {
  try {
    ...
    if (!captchaChallenge || typeof captchaChallenge.num1 === 'undefined' || typeof captchaChallenge.num2 === 'undefined') {
      return res.status(400).json({ error: 'Captcha challenge missing.' });
    }

    const expectedCaptcha = Number(captchaChallenge.num1) + Number(captchaChallenge.num2);
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({ token, email: user.email, role: user.role, username: user.username, userId: user._id });
  } catch (err) {
