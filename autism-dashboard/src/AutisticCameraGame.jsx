...
import { FaCamera, FaSmile, FaMeh, FaAngry, FaSurprise, FaCheckCircle, FaExclamationTriangle } from "react-icons/fa";
...
const ImageWithFallback = ({ src, alt, fallbackIcon }) => {
  ...
  if (error) return <div className="d-flex justify-content-center align-items-center h-100 bg-light">{fallbackIcon}</div>;
  return <img src={src} alt={alt} className="w-100 h-100 object-fit-cover" onError={() => setError(true)} />;
};
...
const AutisticCameraGame = ({ onComplete, onClose, speak, t }) => {
  ...
  const [detectedExpression, setDetectedExpression] = useState("Initializing..."); 
  ...
  useEffect(() => {
    const loadModels = async () => {
      try {
        ...
        if (!testFetch.ok) throw new Error(`Model files not found at ${MODEL_URL}`);
        ...
        setGameStatus("error");
        setErrorMsg("Error loading AI models. Ensure files are in 'public/models'.");
      }
    };
    loadModels();
  }, [speak, t]);
    ...
    if (gameStatus === "playing" && modelsLoaded) {
      loopInterval.current = setInterval(async () => {
        if (webcamRef.current && webcamRef.current.video && webcamRef.current.video.readyState === 4) {
          ...
          // This prevents the "Box.constructor" error when width/height is 0
              ...
              try {
                ...
                if (detections.length > 0) {
                  ...
                  if (maxEmotion === target) {
                    ...
                    matchStreak.current = Math.min(100, matchStreak.current + boost);
                  } else {
                    matchStreak.current = Math.max(0, matchStreak.current - 2);
                  }
  ...
  const handleSuccess = () => {
    ...
    if (speak) {
      const availablePhrases = SUCCESS_PHRASES.filter(p => p !== lastSpokenPhrase.current);
  ...
  return (
    <div className="d-flex flex-column align-items-center justify-content-center bg-dark text-white" style={{ height: "100vh", width: "100vw" }}>
      
      {/* Top HUD */}
      <div className="position-absolute top-0 w-100 p-3 d-flex justify-content-between align-items-center bg-secondary" style={{zIndex: 10}}>
        <div className="d-flex align-items-center">
           <h4><FaCamera className="me-2" />Face Mimic</h4>
           <Badge bg={modelsLoaded ? "success" : "warning"} className="ms-3">
             {modelsLoaded ? "AI Active" : "Loading..."}
           </Badge>
        </div>
        <div>
          <span className="me-4">Score: <strong>{score}</strong></span>
          <Button variant="danger" size="sm" onClick={onClose}>Exit</Button>
        </div>
      </div>

      {/* Error Screen */}
      {gameStatus === "error" && (
        <Alert variant="danger" className="position-absolute top-50 start-50 translate-middle text-center shadow" style={{zIndex: 999, width: '80%', maxWidth: '500px'}}>
           <h4><FaExclamationTriangle className="me-2"/>AI Load Failed</h4>
           <p>{errorMsg}</p>
           <Button variant="outline-danger" onClick={onClose}>Go Back</Button>
        </Alert>
      )}
      ...
      <div className="d-flex w-100 h-100">
        ...
        {/* LEFT: Prompt Area */}
        <div className="w-50 d-flex flex-column align-items-center justify-content-center p-5"
             style={{ background: gameStatus === 'success' ? '#28a745' : '#343a40', transition: '0.5s' }}>
          {gameStatus === 'finished' ? (
             <div className="text-center">
               <h1>🎉 Incredible!</h1>
               <h3>You mastered every emotion!</h3>
               <Button size="lg" variant="light" className="mt-4" onClick={finishGame}>Finish & Save</Button>
             </div>
          ) : (
             <>
               <div className="mb-4 shadow-lg bg-white position-relative" 
                    style={{ width: '300px', height: '300px', borderRadius: '20px', overflow: 'hidden', border: '5px solid white' }}>
                 <ImageWithFallback 
               ...
               <h2 className="display-4 fw-bold text-center text-capitalize">{TARGET_EMOTIONS[currentEmotionIndex].name}</h2>
        ...
        <div className="w-50 position-relative bg-black d-flex align-items-center justify-content-center overflow-hidden">
           ...
           <Webcam
             ...
             style={{ position: 'absolute', width: '100%', height: '100%', objectFit: 'cover', opacity: gameStatus === 'finished' ? 0.2 : 1 }}
             mirrored={true}
           />
           
           {/* AI Visualization Overlay */}
           <div className="position-absolute bottom-0 w-100 p-4" style={{background: 'linear-gradient(to top, rgba(0,0,0,0.9), transparent)'}}>
             <div className="container">
                <div className="row align-items-end">
                    <div className="col-12 text-center mb-2">
                        <span className="text-white-50 small text-uppercase">AI Sees</span>
                        <h2 className={`fw-bold display-4 text-capitalize ${detectedExpression === TARGET_EMOTIONS[currentEmotionIndex].name ? "text-success" : "text-white"}`}>
                            {detectedExpression}
                        </h2>
                    </div>
                    <div className="col-12">
                        ...
                        <ProgressBar 
                            now={matchProgress} 
                            variant={matchProgress > 80 ? "success" : "warning"} 
